...
//	blocks = cuSelect(W.getPassport().problemNumber % 4); //The index of the used video card will be equal to the task number
	...
	cuSetConstants(sizeof(Vortex2D)/sizeof(double), Vortex2D::offsPos / sizeof(double), Vortex2D::offsGam / sizeof(double), Vortex2D::offsSgm / sizeof(double));
	
	n_CUDA_wake = 0;
	inflTreeWake.reset(new BHcu::CudaTreeInfo(blocks, tree_T::vortex, object_T::point4, scheme_T::noScheme, true));
	cntrTreeWake.reset(new BHcu::CudaTreeInfo(blocks, tree_T::contr, object_T::point4, scheme_T::noScheme, false));
#ifdef TURB	
	cntrTreeNut.reset(new BHcu::CudaTreeInfo(blocks, tree_T::contr, object_T::point4, scheme_T::noScheme, false));
#endif	
...
{
#if defined(__CUDACC__) || defined(USE_CUDA)
	inflTreePnlVortex->MemoryFreeForGMRES();
	...
	if (W.getMeasureVP().getWakeVP().vtx.size())
	{
		...
		ReleaseDevMem(W.getMeasureVP().devPressurePtr, 43);
	...
	if (W.getMeasureVP().getWakeVP().vtx.size() > 0)
	{
		...
		if (W.getMeasureVP().getWakeVP().vtx.size() > n_CUDA_velVP)
		{
			...
			if (curLength > 0)
			{
				...
				ReleaseDevMem(W.getMeasureVP().devPressurePtr, 56);
			...
			W.getMeasureVP().devPressurePtr = ReserveDevMem<double, 1>(sz, n_CUDA_velVP);
