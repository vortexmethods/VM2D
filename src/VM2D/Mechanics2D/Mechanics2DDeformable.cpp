...
//deformParam = {f, alpha, lambda, c1, c2};

double Beam::getGivenLaw(double x, double t, const std::vector<double>& deformParam) const //имитатор деформации упругой линии 
{
	const double c1 = deformParam[3];
	const double c2 = deformParam[4];
	double alpha;
    
	if (fsi) //Turek
		alpha = 0.0; //Turek
	else
		alpha = deformParam[1]; //Fish
	

	const double lambda = deformParam[2];
	const double length = 1.0;
	const double f = deformParam[0];
	if (!fsi && (f == 0))
		alpha = 0.0;
...
void MechanicsDeformable::ReadSpecificParametersFromDictionary()
{
	...
	W.getInfo('i') << "mechDeformable: " << "deformParam = {";
	for (auto val : deformParam)
		W.getInfo('i') << " " << val;
	W.getInfo('i') << " }" << std::endl;
