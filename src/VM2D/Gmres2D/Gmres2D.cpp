...
#pragma omp parallel for
		...
		//for (int i = 0; i <= j; ++i)
		//	cudaMemcpy(devViBund + w.size() * i, V[i].data(), w.size() * sizeof(double), cudaMemcpyHostToDevice);


		////////////////////////////////////////////////////////////////////////
		//cudaMemcpy(devViBund, Vflat.data(), (j+1) * w.size() * sizeof(double), cudaMemcpyHostToDevice); //was
		...
		//		w.size(), \
		//		devViBund + w.size() * i, 1,  /* указатель на V[i], шаг 1 */ \
