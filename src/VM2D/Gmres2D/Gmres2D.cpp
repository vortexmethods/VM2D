...
bool GmresSolver::IterRot(const double nrmRhs, double& gs, int m, bool residualShow)
{
	bool fl;
	double buf;

	double epsGMRES = W.getPassport().numericalSchemes.gmresEps;
	...
	if (residualShow)
		std::cout << "Iteration: " << m << ", residual = " << (fabs(gs) / nrmRhs) << std::endl;
...
void GmresSolver::SolCircleRundirect(const std::vector<double>& A, const std::vector<double>& rhs, size_t startRow, size_t startRowReg, size_t np, std::vector<double>& res)
{
...
void GmresSolver::SolMdirect(const std::vector<double>& A, const std::vector<double>& rhs, size_t startRow, size_t startRowReg, size_t np, std::vector<double>& res, bool lin)
{
...
void GmresSolver::SolCircleRun(std::vector<double>& AX, const std::vector<double>& rhs, int p)
{
	const Airfoil& afl = W.getAirfoil(p);
	int n = (int)afl.getNumberOfPanels();
	sweepVectors& sw = allSW[p];
	bool linScheme = (W.getPassport().numericalSchemes.boundaryCondition.second == 2);
...
void GmresSolver::SolM(std::vector<double>& AX, const std::vector<double>& rhs, int p)
{	
	const Airfoil& afl = W.getAirfoil(p);
	int n = (int)afl.getNumberOfPanels();
	sweepVectors& sw = allSW[p];
...
void GmresSolver::PreCalculateCoef(int aflIndex) 
{
	...
	const Airfoil& aflP = W.getAirfoil(aflIndex);
	...
	for (size_t i = 0; i < nPanelsP; ++i)
	{
		...
		for (int j : vecV)
		{
			...
			if (aflP.isAfter(j, i))
				prec[aflIndex][i] = (i00.kcross()) * (1.0 / dj.length());
			if (aflP.isAfter(i, j))
				prea[aflIndex][i] = (i00.kcross()) * (1.0 / dj.length());
			...
			if (linScheme) {
				...
				if (aflP.isAfter(j, i))
					prec1[aflIndex][i] = (i11) * (1.0 / dj.length());
				if (aflP.isAfter(i, j))
					prea1[aflIndex][i] = (i11) * (1.0 / dj.length());
			}
		}
	}
}
	...
	: W(W_)
{
	...
	totalVsize = linScheme ? nTotPan * 2 : nTotPan;
...
GmresSolver::~GmresSolver()
{
	...
	//W.getNonConstCuda().inflTreePnlVortex->MemoryFreeForGMRES();
	cudaFree(devViBund);
	cudaFree(devw);
	//cudaFree(mulptr);
...
#pragma omp parallel for reduction(+:nrmRhs)
	...
	for (size_t i = 0; i < nAfl; ++i)
		Vflat.insert(Vflat.end(), rhs[i].begin(), rhs[i].end());
...
#pragma omp parallel for
		...
		if (j == H.size() - 1)
		{
			std::cout << "Reallocation for GMRES" << std::endl;
			H.resize(j * 2 + 2);
...
#pragma omp for
			for (int q = 0; q < (int)w.size(); ++q)
				w[q] += scal * Vflat[(totalVsize + nAfl) * i + q];//V[i][q];
...
#pragma omp for reduction(+:scal)
				for (int q = 0; q < (int)w.size(); ++q)
					scal += w[q] * Vflat[(totalVsize + nAfl) * i + q];//V[i][q];
