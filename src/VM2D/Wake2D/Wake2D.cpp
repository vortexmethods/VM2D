		...
		for (size_t vt = 0; vt + 1 < vtx.size(); ++vt)
		{
			...
			if (!flag[vt])
			{
				...
				if ((ssd != 0) && (!flag[ssd]))
				{
					...
					sumVtx.sigma() = std::max(vtxI.sigma(), vtxK.sigma());
			...
			if (!flag[vt])
			{
				...
				for (int s = 0; s < knbForRestruct; ++s)
				{
					...
					if (/*(ssd != 0) &&*/ (!flag[ssd])) //ssd==0 always true
					{
						...
						//{
							ri.push_back(Vortex2D(vtxI.r(), vtxI.g(), vtxI.sigma()));
							rj.push_back(Vortex2D(vtxK.r(), vtxK.g(), vtxK.sigma()));
...
void Wake::Restruct()
{
	...
	if (W.getPassport().wakeDiscretizationProperties.epscol > 0)
	{
		...
		bool useFastCollapseAlgorithm = false;
#if defined(__CUDACC__) || defined(USE_CUDA)		
		if (W.getPassport().numericalSchemes.velocityComputation.second == 1)
			useFastCollapseAlgorithm = true;
#endif 

		if (!useFastCollapseAlgorithm)
		{
		...
		else
		{
			...
			for (int collapsStep = 0; collapsStep < 1; ++collapsStep)
			{
				...
				BHcu::CudaTreeInfo knnTree(W.getCuda().blocks, tree_T::contr, object_T::point4, scheme_T::noScheme, false);
				knnTree.MemoryAllocate((int)W.getCuda().n_CUDA_wake);
				knnTree.Update((int)W.getWake().vtx.size(), W.getWake().devVtxPtr);
				knnTree.Build();
			...
			//поиск пар для объединения			
				...
				for (size_t i = 0; i < ri.size(); ++i)
				{
					if (hit[2 * i + 0] == -1 && hit[2 * i + 1] == -1)
					{
						...
						vtx[rindex[i].first].sigma() = std::max(vtx[rindex[i].first].sigma(), vtx[rindex[i].second].sigma());
