...
//	//////////////////////// CUDA ///////////////////////
	...
	if (newPos.size() > 0)
	{
		...
		for (size_t afl = 0; afl < airfoil.size(); ++afl)
		{
			...
			std::vector<int> hit = lbvh_check_inside((int)currentStep, (int)newPos.size(), devNewpos_ptr, (int)airfoil[afl]->getNumberOfPanels(), airfoil[afl]->devRPtr);
...
#pragma omp parallel for
				for (int i = 0; i < (int)airfoil[pi]->getNumberOfPanels(); ++i)
				{
					...
					if (linScheme)
						if (pi == pj)
							w[cntr + i + airfoil[pi]->getNumberOfPanels()] += -V[j][cntr + i + airfoil[pi]->getNumberOfPanels()] * diag[pi][i + airfoil[pi]->getNumberOfPanels()];
				}	
			}
...
//////
	...
	/*
	int nFullVars = getNumberOfBoundary();
	for (int i = 0; i < getNumberOfBoundary(); ++i)
		nFullVars += boundary[i]->GetUnknownsSize();
	...
	for (int i = 1; i < getNumberOfBoundary(); ++i)
		Gpos[i] = Gpos[i - 1] + boundary[i-1]->GetUnknownsSize();

	std::vector<int> Gvsize(getNumberOfBoundary());
	for (int i = 0; i < getNumberOfBoundary(); ++i)
		Gvsize[i] = boundary[i]->GetUnknownsSize();
	...
	//GMRES_Direct(nFullVars, getNumberOfBoundary(), Gmatr, Grhs, Gn, Gpos, Gvsize, Ggam, GR);
	...
	int niter;
	GMRES(Ggam, GR, GGrhs, GrhsReg, niter, passport.numericalSchemes.boundaryCondition.second);
	...
	//std::ofstream solFile(getPassport().dir + "/dbg/sol0.txt");
