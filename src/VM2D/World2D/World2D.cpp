		...
		if (!semiImplicitStrategy)
		{
			...
			if (getPassport().numericalSchemes.velocityComputation.second == 0 && getPassport().numericalSchemes.linearSystemSolver.second == 2)
			{
				...
				if (nTotPan > 0)
				{
					...
					treePnlVrt.UpwardTraversal(getPassport().numericalSchemes.gmresMultipoleOrder);
				}
#endif
			}
				...
				if (nTotPan > 0)
				{
					...
					{					
						...
						if (isAnyMovableOrDeformable())
						{
							...
							treePnlSrc.Build();
							treePnlSrc.UpwardTraversal(getPassport().numericalSchemes.nbodyMultipoleOrder);
						}
					}
			...
			if (nTotPan > 0 && getPassport().numericalSchemes.velocityComputation.second == 1)
			{
				...
				getCuda().inflTreePnlVortex->UpwardTraversal(getPassport().numericalSchemes.nbodyMultipoleOrder);
#endif
			}
			timerInitialBuild.stop();
...
void World2D::CalcVortexVelo()
{
	...
	if (!(currentStep % 1))
	{
		VMlib::CreateUserDirectory(passport.dir, "dbg");
