...
double VelocityBiotSavart::CalcConvVeloToSetOfPointsFromWake(const WakeDataBase& pointsDb, std::vector<Point2D>& velo, std::vector<double>& domainRadius, bool calcVelo, bool calcRadius)
{
...
#pragma omp parallel for default(none) shared(selfVelo, cft, calcVelo, calcRadius, pointsDb, domainRadius) private(tempVel, velI, dst2, dst2eps) schedule(dynamic, DYN_SCHEDULE)
		for (int i = 0; i < pointsDb.vtx.size(); ++i)
		{
			...
			for (size_t j = 0; j < W.getWake().vtx.size(); ++j)
			{
				...
				const double sigmaJ2 = sqr(W.getWake().vtx[j].sigma());

				tempVel.toZero();
				dst2eps = VMlib::boundDenom(dst2, sigmaJ2); //Сглаживать надо!!!				
			...
			for (size_t j = 0; j < W.getSource().vtx.size(); ++j)
			{
				...
				const double sigmaJ2 = sqr(W.getSource().vtx[j].sigma());
...
#if defined(USE_CUDA)
double VelocityBiotSavart::GPUCalcConvVeloToSetOfPointsFromWake(std::unique_ptr<BHcu::CudaTreeInfo>& cntrTree, const WakeDataBase& pointsDb, std::vector<Point2D>& velo, std::vector<double>& domainRadius, bool calcVelo, bool calcRadius)
{
	VMlib::vmTimer timerA;
