...
void Velocity::GPUDiffVeloFAST(const std::vector<double>& domainRadius, std::vector<double>& I1, std::vector<Point2D>& I2)
{
	...
	if (nvt > 0)
	{
		...
		treeWake.Update((int)W.getWake().vtx.size(), W.getWake().devVtxPtr);
		treeWake.Build();
...
void Velocity::SaveVisStress()
{
		...
		//		if ((W.getPassport().timeDiscretizationProperties.saveVTK > 0) && (W.ifDivisible(10)) && (W.getNumberOfAirfoil() > 0))
	{
		...
		{
			if (q == 0)
				VMlib::CreateUserDirectory(W.getPassport().dir, "visStress");
...
void Velocity::GPUGetWakeInfluenceToRhs(const Airfoil& afl, std::vector<double>& wakeVelo) const
{
	...
	if (afl.numberInPassport == 0)
	{
		...
		if ((nvt > 0) || (nsr > 0))
		{
			cuCalculateRhs(nTotPan, dev_ptr_pt, nvt, dev_ptr_vt, nsr, dev_ptr_sr, dev_ptr_rhs, dev_ptr_rhsLin);
...
void Velocity::CalcConvVelo()
{
	...
	VMlib::vmTimer timerAgpu, timerBgpu;
	...
	if (W.getPassport().numericalSchemes.velocityComputation.second == 1)
	{
		...
		cntrTree->Update((int)W.getWake().vtx.size(), W.getWake().devVtxPtr);
		cntrTree->Build();
	...
	double timerCgpuduration = GPUCalcConvVeloToSetOfPointsFromWake(W.getNonConstCuda().cntrTreeWake, W.getWake(), wakeVortexesParams.convVelo, wakeVortexesParams.epsastWake, true, true);
	...
	/// FMM ///
	/*
	std::vector<fmm::particle2d> sources(W.getWake().vtx.size());
	#pragma omp parallel for
	for (int i = 0; i < (int)sources.size(); ++i)
	{
		const auto& w = W.getWake().vtx[i];
	
		sources[i].center = fmm::point2d(w.r()[0], w.r()[1]);
		sources[i].q = w.g();
		sources[i].morton_code = 0;
	}
	...
	//fmm::ComputeExact<fmm::InteractionType::Potential2d>(sources, targets); // or fmm::gpu::ComputeExact... for gpu version
	fmm::ComputeExact<fmm::InteractionType::Force2d>(sources, targets); // or fmm::gpu::ComputeExact... for gpu version
	std::cout << "Compute exact time = " << omp_get_wtime() - t << std::endl;
	
	t = omp_get_wtime();	

	fmm::FastMultipole fmm(sources, targets, 1.e-8, fmm::FMM_AUTO, fmm::FMM_AUTO); // or fmm::gpu::FastMultipole... for gpu version
	fmm::ReadError<fmm::InteractionType::Force2d>(fmm.forces);
	//fmm::ReadError<fmm::InteractionType::Potential2d>(fmm.potentials);	
	std::cout << "Total time = " << omp_get_wtime() - t << std::endl;
	*/
	...
	if (W.getCurrentStep() == 0)
	{
		...
		treeTimeFile << "step,time,N,lev,averPoints,tBUI,tUPW,tDNW,tBUIgpu,tUPWgpu,tDNWgpu\n";
	}
	else
		treeTimeFile.open(W.getPassport().dir + "/dbg/treeTime.csv", std::ios::app);


	int estLevel = std::max(4, (int)(log2(W.getWake().vtx.size())));

	for (int controlLevel = std::max(estLevel - 2, 1); controlLevel <= estLevel + 2; ++controlLevel)
	{
		VMlib::vmTimer timerA, timerB;
		if (W.getWake().vtx.size() > 0)
		{
			if (W.getPassport().numericalSchemes.velocityComputation.second == 1)
			{
				W.getInflTreeWake().Update(W.getWake().vtx, controlLevel);
				timerA.start();
				W.getInflTreeWake().Build();
				timerA.stop();
				timerB.start();
				W.getInflTreeWake().UpwardTraversal(W.getPassport().numericalSchemes.nbodyMultipoleOrder);
				timerB.stop();

				W.getCntrTreeWake().Update(W.getWake().vtx, controlLevel);
				W.getCntrTreeWake().Build();
				W.getCntrTreeWake().UpwardTraversal(W.getPassport().numericalSchemes.nbodyMultipoleOrder);
			}

			std::vector<Point2D> tempVelo(wakeVortexesParams.convVelo.size());
			std::vector<double> tempRadius(wakeVortexesParams.epsastWake.size());

			//Вызов виртуальной функции				
			double timerCduration = CalcConvVeloToSetOfPointsFromWake(W.getWake(), tempVelo, tempRadius, true, false);

			int nObjInContr = 0;
			for (int c = 0; c < W.getCntrTreeWake().indexControlCells.size(); ++c)
			{
				const auto& idx = W.getCntrTreeWake().indexControlCells[c];
				if (idx > W.getCntrTreeWake().object.size())
					nObjInContr += 1;
				else
				{
					nObjInContr += W.getCntrTreeWake().range[idx].second - W.getCntrTreeWake().range[idx].first + 1;
				}
			}
			
			treeTimeFile << W.getCurrentStep() << ',' << W.getCurrentTime() << ',' << W.getInflTreeWake().object.size() << ',' \
				<< controlLevel << ',' << (double)nObjInContr / W.getCntrTreeWake().indexControlCells.size() << ',' \
				<< timerA.duration() << ',' << timerB.duration() << ',' << timerCduration << ',' \
				<< timerAgpu.duration() << ',' << timerBgpu.duration() << ',' << timerCgpuduration << '\n';
		}
	}
	...
	*/
	...
	if (W.getWake().vtx.size() > 0)
	{
		if (W.getPassport().numericalSchemes.velocityComputation.second == 1)
		{
			...
			timeUpw1 = W.getInflTreeWake().UpwardTraversal(W.getPassport().numericalSchemes.nbodyMultipoleOrder);

			//timeUpd2 = W.getCntrTreeWake().Update(W.getWake().vtx, estOptLevel);
			//timeBld2 = W.getCntrTreeWake().Build();
			//timeUpw2 = W.getCntrTreeWake().UpwardTraversal(W.getPassport().numericalSchemes.nbodyMultipoleOrder);
			
			//std::cout << "timeUpd = " << timeUpd1 << " " << timeUpd2 << '\n';
			//std::cout << "timeBld = " << timeBld1 << " " << timeBld2 << '\n';
			//std::cout << "timeUpw = " << timeUpw1 << " " << timeUpw2 << '\n';
	...
	if (W.getWake().vtx.size() > 0)
	{
		...
		std::cout << "Hybrid time ConvVelo Wake = " << fullTimer.duration() <<
			//W.timerConvVelo.duration() << 
			" ( " << timeUpd1 + timeUpd2 + timeBld1 + timeBld2 + timeUpw1 + timeUpw2 + timeDnw << " )" << '\n';

		fmm::ReadErrorHyb(*((std::vector<std::array<double, 2>>*) &wakeVortexesParams.convVelo));
		std::cout << "Exact time = " << timerExact.duration() << '\n';
	}
	//*/
...
#ifdef OPTIMIZER	
		if ((W.getCurrentStep() >= OPTIMIZER_START_STEP && W.getCurrentStep() <= OPTIMIZER_STOP_STEP) || W.getCurrentStep() == 0)
			CalcVeloToWakeVP();
#else
	...
	CalcVeloToWakeVP();
	//t1 += omp_get_wtime();
	//std::cout << "time ConvVelo VP = " << t1 << std::endl;
#endif
...
void Velocity::CalcVeloToWakeVP()
{
	...
	case 1:
		...
		{			
			...
			cntrTree->Update((int)W.getMeasureVP().getWakeVP().vtx.size(), W.getMeasureVP().getWakeVP().devVtxPtr);
			cntrTree->Build();	
	...
	case 1:
		...
		W.getCntrTreeVP().Update(W.getMeasureVP().getWakeVP().vtx);
		W.getCntrTreeVP().Build();
		W.getCntrTreeVP().UpwardTraversal(W.getPassport().numericalSchemes.nbodyMultipoleOrder);
		W.getInflTreeWake().DownwardTraversalVorticesToPoints(W.getCntrTreeVP(), velConvWake, W.getNonConstMeasureVP().getNonConstDomainRadius(), theta, order, false);
	}
#endif	
