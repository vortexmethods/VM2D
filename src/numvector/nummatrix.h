...
public:
	...
	template <typename P>
	auto operator+(const nummatrix<P, n, m>& y) const -> nummatrix<std::remove_const<decltype(r[0] + y.r[0])>::type, n, m>
	{
		nummatrix<std::remove_const<decltype(r[0] + y.r[0])>::type, n, m> res;
		for (size_t i = 0; i < n*m; ++i)
			res[0][i] = r[i] + y[i];
		return res;
	}//operator+
	...
	template <typename P>
	auto operator*(const P с) const -> nummatrix<std::remove_const<decltype(r[0] * c)>::type, n, m>
	{
		nummatrix<std::remove_const<decltype(r[0] * c)>::type, n, m> res;
		for (size_t i = 0; i < n*m; ++i)
			res[0][i] = c * r[i];
		return res;
	}//operator*
	...
	auto norm1() const -> std::remove_const<std::remove_reference<decltype(r[0])>::type>::type
	{
		std::vector<std::remove_const<std::remove_reference<decltype(r[0])>::type>::type> ms(m, 0);
	...
	auto norminf() const -> std::remove_const<std::remove_reference<decltype(r[0])>::type>::type
	{
		std::vector<std::remove_const<std::remove_reference<decltype(r[0])>::type>::type> ms(n, 0);
		for (size_t i = 0; i < n; ++i)
		for (size_t j = 0; j < m; ++j)
			ms[i] += abs(r[i*m + j]);
		return std::max_element(ms.begin(), ms.end());
	...
	/// Транспонирование
	inline nummatrix<T, m, n> transpose()
	{
		nummatrix<T, m, n> res;
		for (size_t i = 0; i < n; ++i)
		for (size_t j = 0; j < m; ++j)
	...
	auto operator&(const numvector<P, m>& x) -> \
		numvector<std::remove_const<decltype(r[0] * x[0])>::type, n>
	{
		numvector<std::remove_const<decltype(r[0] * x[0])>::type, n> res;
	...
	auto operator&(const nummatrix<P, m, p>& B) -> \
		nummatrix<std::remove_const<decltype(r[0] * x[0])>::type, n, p>
	{
		nummatrix<std::remove_const<decltype(r[0] * x[0])>::type, n, p> res(0);
...
numvector<std::remove_const<decltype(r[0] * x[0])>::type, m>
{
	numvector<std::remove_const<decltype(r[0] * x[0])>::type, m> res;
	for (size_t i = 0; i < m; ++i)
	{
		res[i] = 0;
		for (size_t j = 0; j < n; ++j)
...
nummatrix<std::remove_const<decltype(x[0] * y[0])>::type, n, m>
{
	nummatrix<std::remove_const<decltype(x[0] * y[0])>::type, n, m> res;
