...
#pragma warning (pop)
	...
	inline void CreateUserDirectory(const std::string& dir, const std::string& name)
	{
#if defined(_WIN32)
	...
	/// \brief Возведение числа в куб
	/// 
	/// \tparam T тип данных
	/// \param[in] x аргумент
	/// \return квадрат аргумента \f$ x^3 \f$
	template<typename T>
	inline T cubPower(T x) { return x * x * x; }
