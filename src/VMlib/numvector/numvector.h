...
namespace VMlib
{
	...
	*/
	...
	public:
		...
		/// \brief Нешаблонный оператор копирования
		///
		/// \tparam T тип данных компонент вектора
		/// \tparam n длина вектора
		/// \param[in] vec константная ссылка на копируемый вектор
		HD numvector<T,n>& operator=(const numvector<T, n>& vec)
		{
			//for (size_t i = 0; i < n; ++i)
			//	r[i] = vec[i];
			memcpy(this->data, vec.data, n * sizeof(T));
		...
		/// \brief Шаблонный оператор копирования
		///
		/// \tparam P тип данных копируемого вектора
		/// \tparam n длина вектора
		/// \param[in] vec константная ссылка на копируемый вектор
		template <typename P>
		HD numvector<T,n> operator=(const numvector<P, n>& vec)
		{
			for (size_t i = 0; i < n; ++i)
				this->data[i] = vec[i];
		}//numvector(...)
