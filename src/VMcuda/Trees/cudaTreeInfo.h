...
/*!
\file
\brief Заголовок класса дерева для реализации быстрых алгоритмов на CUDA
...
#ifndef CUDATREEINFO_H
#define CUDATREEINFO_H
...
{    
    ...
    */
    struct infoForMatVecMul
    {
        ...
        infoForMatVecMul()
        {
            i00D = i01D = i10D = i11D = nullptr;//
            //nClosePanelsD = nullptr;
            //nFarCellsD = nullptr;
            closeCellsIdxD = nullptr;//
            farCellsIdxD = nullptr; //
            //closePrefixSumD = nullptr;
            //farPrefixSumD = nullptr;
        }
    ...
    /*!
    \brief Класс, определяющий структуры дерева для быстрых методов при их реализации на GPU
    ...
    */
    ...
    public:
        ...
        int* massD;	//массы (единица для точечного вихря, число вихрей для ячейки)

        Point2D* maxrD, * minrD;      //габаритный прямоугольник
        Point2D* momsD;               //мультипольные моменты всех ячеек; хранятся в виде <mom_0x, mom_0y=0, mom_1x, mom_1y, ..., mom_px, mom_py>, <для второй ячейки> ...
        Point2D* ED; //к-ты локальных разложений

        //For Morton tree
        int* mortonCodesKeyUnsortD;
        int* mortonCodesKeyD;

        int* mortonCodesIdxUnsortD; //0 1 2 3 ... nbodies-1		
        int* mortonCodesIdxD;

        int* levelUnsortD;
        int* levelSortD;

        int* indexUnsortD;  //0 1 2 3 ... nbodies-2
        int* indexSortD;
        int* indexSortTD;

        Point2D* centerD;  //Положения внутренних узлов в дерева Карраса
        Point4D* lowerupperD; //Левый нижний и правый верхний углы ячейки
        Point4D* gabForLeavesD; //Левый нижний и правый верхний углы листа

        int* parentD;     //Номер ячейки-родителя
        long long* childD;  //Потомки внутренних ячеек (в одной ячейке храним сразу два целых числа)
        long long* rangeD;  //Диапазон частиц во внутренней ячейке (в одной ячейке храним сразу два целых числа)
        ...
        CudaTreeInfo(int nBlock_, tree_T treeType_, object_T objectType_, scheme_T schemeType_, bool duplicateObj_);
        ~CudaTreeInfo();

        void Update(int nObject_, double* objectD_);
        ...
        float MemoryAllocateForGMRES(bool firstStep, double itheta2);
        ...
        float DownwardTraversalGMRES(double* resD, double* resLinD, double itheta2, int order, int iter, bool memAlloc);
