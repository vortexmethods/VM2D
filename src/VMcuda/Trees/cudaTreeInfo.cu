...
namespace BHcu
{
    ...
    void infoForMatVecMul::ReleaseGpuMemory()
    {
        if (i00D)
        {
            cudaFree(i00D);
        ...
        if (closeCellsIdxD)
        {
            cudaFree(closeCellsIdxD);
            closeCellsIdxD = nullptr;
        }

        if (farCellsIdxD)
        {
            cudaFree(farCellsIdxD);
            farCellsIdxD = nullptr;
        }
    }


    __global__
        void FillGabForPointsKernel(int n, const double* __restrict pntD, int sizeOfElement, double4* __restrict gabD)
    {
        ...
        if (i < n)
        {
            ...
            gabD[i] = make_double4(p.x - sgm, p.y - sgm, p.x + sgm, p.y + sgm);
        }
    }
    ...
    void CudaTreeInfo::Update(int nObject_, double* objectD_)
    {
        ...
        if (nObject > 0)
        {
            ...
            FillGabForPointsKernel << <(nObject + 31) / 32, 32 >> > (nObject, (double*)objectD, sizeOfElement, (double4*)gabForLeavesD);
    ...
    float CudaTreeInfo::DownwardTraversalVorticesToPoints(CudaTreeInfo& cntrTree, Point2D* velD, double* epsastD, double theta, int order, bool calcRadius)
    {
        ...
        if (nObject > 0)
        {
            ...
            BHcu::treeVorticesToPointsCalculationKernel<12, NUMBER_OF_THREADS> << <nBlock * FACTORforces, NUMBER_OF_THREADS >> > (
    ...
    float CudaTreeInfo::DownwardTraversalVorticesToPanels(CudaTreeInfo& cntrTree, double* rhsD, double* rhsLinD, double theta, int order)
    {
        ...
        if (nObject > 0)
        {
            ...
            treeRhsCalculationKernel<12> << <blocks * FACTORrhs, th5rhs >> > (nNode, nObject, itheta2, (int2*)childD, \
                (double2*)momsD, (double4*)objectD, mortonCodesIdxD, (double2*)centerD, \
    ...
    float CudaTreeInfo::DownwardTraversalPanelsToPoints(CudaTreeInfo& cntrTree, Point2D* velD, double theta,
        int order)
    {
        float time = 0.0f;
        if (nObject > 0)
        {
            double itheta2 = 1.0 / (theta * theta);
            time = treePanelsToPointsCalculationWrapper(*this, cntrTree, order, itheta2, velD);
        }
        return time;
    }

    float CudaTreeInfo::MemoryAllocateForGMRES(bool firstStep, double itheta2)
    {
        if (firstStep)
        {
        ...
        matVecMulInfo.ReleaseGpuMemory();
    ...
    float CudaTreeInfo::DownwardTraversalGMRES(double* resD, double* resLinD, double itheta2, int order, int iter, bool memAlloc)
    {        
        cudaEvent_t start, stop;
        float time;
        ...
        if (iter == 0)
        {
            BHcu::treeMatrToVecZeroIterKernel12<12> << <nBlock * FACTORslae, THREADSslae >> > (
                nNode, nObject, itheta2,
                (int2*)childD, (double2*)momsD,
                (double*)gabForLeavesD,
                (double*)objectD,
                mortonCodesIdxD,
                (double2*)centerD,
                indexSortD, indexSortTD, (double4*)lowerupperD,
                schemeType,
                nObject, (double*)gabForLeavesD, (double2*)ED,
                mortonCodesIdxD,
                resD, resLinD,
                matVecMulInfo.closeCellsIdxD, matVecMulInfo.closePrefixSumD,
                matVecMulInfo.farCellsIdxD, matVecMulInfo.farPrefixSumD,
                (double2*)matVecMulInfo.i00D, (double2*)matVecMulInfo.i01D, (double2*)matVecMulInfo.i10D, (double2*)matVecMulInfo.i11D, iter);

            CudaTestError("treeMatrToVecZeroIterKernel12 launch failed");
        }
        else
        {
            BHcu::treeMatrToVecNonZeroIterKernel12<12> << <nBlock * FACTORslae, THREADSslae >> > (
                nNode, nObject, (double2*)momsD,
                (double*)gabForLeavesD,
                (double*)objectD,
                mortonCodesIdxD,
                (double2*)centerD, indexSortTD,
                schemeType,
                nObject, (double*)gabForLeavesD, (double2*)ED,
                mortonCodesIdxD,
                resD, resLinD,
                matVecMulInfo.nClosePanelsD,
                matVecMulInfo.nFarCellsD,
                matVecMulInfo.closeCellsIdxD, matVecMulInfo.closePrefixSumD,
                matVecMulInfo.farCellsIdxD, matVecMulInfo.farPrefixSumD,
                (double2*)matVecMulInfo.i00D, (double2*)matVecMulInfo.i01D, (double2*)matVecMulInfo.i10D, (double2*)matVecMulInfo.i11D);
    ...
    float CudaTreeInfo::I1I2CalculationWrapper(double minRd, double* __restrict I1D, Point2D* __restrict I2D, double* __restrict epsastD)
    {
        ...
        treeI1I2CalculationKernel << <nBlock * FACTORI1I2, THREADSI1I2 >> > (
            ...
            (double4*)objectD, mortonCodesIdxD,
    ...
    float CudaTreeInfo::I0I3CalculationWrapper(double minRd, float* __restrict I0D, Point2Df* __restrict I3D, double* __restrict epsastD, const double* __restrict meanEpsD, int nPan, double* __restrict panD, double* __restrict visstrD)
    {
        ...
        ClearI0I3 << <nBlock * FACTOR1, THREADS1024 >> > (nObject, I0D, (float2*)I3D);
