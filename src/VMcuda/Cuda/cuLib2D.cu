...
void cuCalculateRhs(size_t myDisp, size_t myLen, size_t npt, double* pt, size_t nvt, double* vt, size_t nsr, double* sr, double* rhs, double* rhsLin)
{
	dim3 blocks(cuCalcBlocks(myLen)), threads(CUBLOCK);
	
	CU_calc_RHS << < blocks, threads >> > (myDisp, myLen, npt, pt, nvt, vt, nsr, sr, rhs, rhsLin);
