...
__device__
thrust::pair<thrust::pair<unsigned int, Real>, Real> query_device(int idx, int timeStep,
        const detail::basic_device_bvh<Real, Objects, IsConst>& bvh,
        const query_nearest<Real>& q, DistanceCalculator calc_dist) noexcept
{
    ...
    *stack_ptr++ = thrust::make_pair(0, mindist(bvh.aabbs[0], q.target));
    ...
    //{
    //    printf("%f, %f, %f, %f, %f, %f\n",
    //        bvh.aabbs[0].lower.x, bvh.aabbs[0].lower.y, bvh.aabbs[0].upper.x, bvh.aabbs[0].upper.y, q.target.x, q.target.y);
    //    printf("mindist(bvh.aabbs[0], q.target) = %f\n", mindist(bvh.aabbs[0], q.target));
    //}
    ...
    do
    {
        ...
        //if ((timeStep >= 1401) && (idx == 29441))
        //    printf("node.second = %f, dist_to_nearest_object = %f\n", node.second, dist_to_nearest_object);
        ...
        if(L_mindist <= R_minmaxdist) // L is worth considering
        {
            ...
            {
                const thrust::pair<real_type, real_type> dist = calc_dist(q.target, bvh.objects[obj_idx]);
    ...
    while (stack < stack_ptr);
    return thrust::make_pair(thrust::make_pair(nearest, frac), dist_to_nearest_object);
}
