...
__device__ __host__
inline float mindist(const aabb<float>& lhs, const float2& rhs) noexcept
{
...
__device__ __host__
inline double mindist(const aabb<double>& lhs, const double2& rhs) noexcept
{
...
__device__ __host__
inline float minmaxdist(const aabb<float>& lhs, const float2& rhs) noexcept
{
    ...
    const float dz = rM_sq.x + rM_sq.y;
    return ::fminf(dx, ::fminf(dy, dz));
}

__device__ __host__
inline double minmaxdist(const aabb<double>& lhs, const double2& rhs) noexcept
{
    ...
    const double dz = rM_sq.x + rM_sq.y;
