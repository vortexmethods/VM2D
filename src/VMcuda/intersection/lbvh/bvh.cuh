        ...
        const unsigned int num_leaves, unsigned int idx)
{
    ...
    if(0 <= i_tmp && (unsigned)i_tmp < num_leaves)
    {
...
// a set of pointers to use it on device.
template<typename Real, typename Object, bool IsConst>
struct basic_device_bvh2;
template<typename Real, typename Object>
struct basic_device_bvh2<Real, Object, false>
{
    using real_type  = Real;
    using aabb2_type  = aabb2<real_type>;
    using node_type  = detail::node;
    using index_type = std::uint32_t;
    using object_type = Object;
...
template<typename Real, typename Object>
struct basic_device_bvh2<Real, Object, true>
{
    using real_type  = Real;
    using aabb2_type  = aabb2<real_type>;
    using node_type  = detail::node;
    ...
    node_type   const* nodes;
    aabb2_type   const* aabbs2;
    object_type const* objects;
};

template<typename Real, typename Object, bool IsConst, typename UInt>
void construct_internal_nodes2(const basic_device_bvh2<Real, Object, IsConst>& self,
        UInt const* node_code, const unsigned int num_objects)
{
    thrust::for_each(thrust::device,
        thrust::make_counting_iterator<unsigned int>(0),
...
template<typename Real, typename Object>
struct default_morton_code_calculator2
{
    default_morton_code_calculator2(aabb2<Real> w): whole(w) {}
    default_morton_code_calculator2()  = default;
    ~default_morton_code_calculator2() = default;
    default_morton_code_calculator2(default_morton_code_calculator2 const&) = default;
    default_morton_code_calculator2(default_morton_code_calculator2&&)      = default;
    default_morton_code_calculator2& operator=(default_morton_code_calculator2 const&) = default;
    default_morton_code_calculator2& operator=(default_morton_code_calculator2&&)      = default;

    __device__ __host__
    inline unsigned int operator()(const Object&, const aabb2<Real>& box) noexcept
    {
        auto p = centroid2(box);
        p.x -= whole.lower.x;
        p.y -= whole.lower.y;
  ...
  public:
    ...
    void construct()
    {
        ...
        thrust::for_each(thrust::device,
            ...
            [self, flags] __host__ __device__ (index_type idx)
            {
#ifdef __CUDA_ARCH__
...
using  bvh_device2 = detail::basic_device_bvh2<Real, Object, false>;
template<typename Real, typename Object>
using cbvh_device2 = detail::basic_device_bvh2<Real, Object, true>;

template<typename Real, typename Object, typename AABB2Getter,
         typename MortonCodeCalculator2 = default_morton_code_calculator2<Real, Object>>
...
class bvh2
{
  public:
    ...
    using aabb2_type   = aabb2<real_type>;
    using node_type   = detail::node;
    using aabb2_getter_type  = AABB2Getter;
    using morton_code_calculator_type = MortonCodeCalculator2;

  public:

    template<typename InputIterator>
    bvh2(InputIterator first, InputIterator last, bool query_host_enabled = false)
        : objects_h_(first, last), objects_d_(objects_h_),
          query_host_enabled_(query_host_enabled)
    {
        this->construct();
    ...
    bvh_device2<real_type, object_type> get_device_repr2()       noexcept
    {
        return bvh_device2<real_type, object_type>{
            static_cast<unsigned int>(nodes_.size()),
            static_cast<unsigned int>(objects_d_.size()),
            nodes_.data().get(), aabbs2_.data().get(), objects_d_.data().get()
        };
    }
    cbvh_device2<real_type, object_type> get_device_repr2() const noexcept
    {
        return cbvh_device2<real_type, object_type>{
            static_cast<unsigned int>(nodes_.size()),
            static_cast<unsigned int>(objects_d_.size()),
            nodes_.data().get(), aabbs2_.data().get(), objects_d_.data().get()
        };
    ...
    void construct2()
    {
        ...
        const auto inf = std::numeric_limits<real_type>::infinity();
        aabb2_type default_aabb2;
        default_aabb2.upper.x = -inf; default_aabb2.lower.x = inf;
        default_aabb2.upper.y = -inf; default_aabb2.lower.y = inf;

        this->aabbs2_.resize(num_nodes, default_aabb2);

        thrust::transform(this->objects_d_.begin(), this->objects_d_.end(),



                aabbs2_.begin() + num_internal_nodes, aabb2_getter_type());

        

        const auto aabb2_whole = thrust::reduce(





            aabbs2_.begin() + num_internal_nodes, aabbs2_.end(), default_aabb2,
            [] __device__ __host__ (const aabb2_type& lhs, const aabb2_type& rhs) {
                return merge2(lhs, rhs);
            });

        

        thrust::device_vector<unsigned int> morton2(num_objects);
        thrust::transform(this->objects_d_.begin(), this->objects_d_.end(),

            aabbs2_.begin() + num_internal_nodes, morton2.begin(),
            morton_code_calculator_type(aabb2_whole));
        ...
        // keep indices ascending order
        thrust::stable_sort_by_key(morton2.begin(), morton2.end(),
            thrust::make_zip_iterator(
                thrust::make_tuple(aabbs2_.begin() + num_internal_nodes,
                                   indices.begin())));
        ...
        //*
        thrust::device_vector<unsigned long long int> morton64(num_objects);
        const auto uniqued = thrust::unique_copy(morton2.begin(), morton2.end(),
                                                 morton64.begin());

        const bool morton_code_is_unique = (morton64.end() == uniqued);
        if(!morton_code_is_unique)
        {
            thrust::transform(morton2.begin(), morton2.end(), indices.begin(),
                morton64.begin(),
                [] __device__ (const unsigned int m, const unsigned int idx)
                {
                    unsigned long long int m64 = m;
        ...
        const auto self = this->get_device_repr2();
        if(morton_code_is_unique)
        {
            const unsigned int* node_code = morton2.data().get();
            detail::construct_internal_nodes2(self, node_code, num_objects);
        }
        else // 64bit version
        {
            const unsigned long long int* node_code = morton64.data().get();
        ...
        thrust::for_each(thrust::device,
            ...
            [self, flags] __host__ __device__ (index_type idx)
            {
                ...
                while(parent != 0xFFFFFFFF) // means idx == 0
                {
                    ...
                    const auto lidx = self.nodes[parent].left_idx;
                    const auto ridx = self.nodes[parent].right_idx;
                    const auto lbox = self.aabbs2[lidx];
                    const auto rbox = self.aabbs2[ridx];
                    self.aabbs2[parent] = merge2(lbox, rbox);
    ...
    thrust::host_vector<node_type> const& nodes_host() const noexcept {return nodes_h_;}
    thrust::host_vector<node_type>&       nodes_host()       noexcept {return nodes_h_;}
    thrust::host_vector<aabb2_type> const& aabbs2_host() const noexcept {return aabbs2_h_;}
    thrust::host_vector<aabb2_type>&       aabbs2_host()       noexcept {return aabbs2_h_;}

  private:

    thrust::host_vector  <object_type>   objects_h_;
    thrust::device_vector<object_type>   objects_d_;
    thrust::host_vector  <aabb2_type>     aabbs2_h_;
    thrust::device_vector<aabb2_type>     aabbs2_;
    thrust::host_vector  <node_type>     nodes_h_;
    thrust::device_vector<node_type>     nodes_;
    bool query_host_enabled_;
};
...
template<typename Real, typename Object>
using  bvh_deviceS = detail::basic_device_bvh2<Real, Object, false>;
template<typename Real, typename Object>
using cbvh_deviceS = detail::basic_device_bvh2<Real, Object, true>;

template<typename Real, typename Object, typename AABBSGetter,
         typename MortonCodeCalculator2 = default_morton_code_calculator2<Real, Object>>
...
class bvhS
{
  public:
    ...
    using aabb2_type   = aabb2<real_type>;
    using node_type   = detail::node;
    using aabbS_getter_type  = AABBSGetter;
    using morton_code_calculator_type = MortonCodeCalculator2;

  public:

    template<typename InputIterator>
    bvhS(InputIterator first, InputIterator last, bool query_host_enabled = false)
        : objects_h_(first, last), objects_d_(objects_h_),
          query_host_enabled_(query_host_enabled)
    {
        this->constructS();
    ...
    bvhS()                      = default;
    ~bvhS()                     = default;
    bvhS(const bvhS&)            = default;
    bvhS(bvhS&&)                 = default;
    bvhS& operator=(const bvhS&) = default;
    bvhS& operator=(bvhS&&)      = default;
    ...
    bvh_deviceS<real_type, object_type> get_device_reprS()       noexcept
    {
        return bvh_deviceS<real_type, object_type>{
            static_cast<unsigned int>(nodes_.size()),
            static_cast<unsigned int>(objects_d_.size()),
            nodes_.data().get(), aabbs2_.data().get(), objects_d_.data().get()
        };
    }
    cbvh_deviceS<real_type, object_type> get_device_reprS() const noexcept
    {
        return cbvh_deviceS<real_type, object_type>{
            static_cast<unsigned int>(nodes_.size()),
            static_cast<unsigned int>(objects_d_.size()),
            nodes_.data().get(), aabbs2_.data().get(), objects_d_.data().get()
        };
    ...
    void constructS()
    {
        ...
        thrust::transform(this->objects_d_.begin(), this->objects_d_.end(),
                ...
                aabbs2_.begin() + num_internal_nodes, aabbS_getter_type());
        ...
        const auto self = this->get_device_reprS();
        if(morton_code_is_unique)
        {
            const unsigned int* node_code = morton2.data().get();
            detail::construct_internal_nodes2(self, node_code, num_objects);
