...
void I0I3CalculationKernel2(
    ...
    const real2* __restrict Mlowerd,
    const real2* __restrict Mupperd,
    const real* __restrict epsast, const real* __restrict meanEpsd, const int npan, const real* __restrict pansd, real* __restrict visstrd, float3* __restrict i0i3Analyt)
    ...
    for (k = threadIdx.x + blockIdx.x * blockDim.x; k < npan; k += blockDim.x * gridDim.x)
    {
        ...
        do
        {
            ...
            while (pd < 2)
            {
                ...
                bool cond = isVortex ? false : ((low.x < p.x) && (p.x < up.x) && (low.y < p.y) && (p.y < up.y));
                                
                if (__all_sync(0xffffffff, ((d > 50.0f * lenj) || (fabsf(s) > 50.0f * lenj)) && !(cond)))
                {
                ...
                {                    
                    ...
                    if ((d > 5.0f * lenj) || (fabsf(s) > 5.0f * lenj))
                    {
                        xix = q.x * iDDomRad;
                        xiy = q.y * iDDomRad;
...
(
    ...
    const int* __restrict MmortonCodesIdxd,
    const real2* __restrict Mposd, const int* __restrict MindexSortd, const int* __restrict MindexSortTd,
    const int npointsd, const real* __restrict dev_ptr_pt, const real2* __restrict pointsd, real2* __restrict Ed,
    const int* __restrict MmortonCodesIdxPointsd,
  ...
  /******************************************************************************/
        ...
        real* __restrict visstr)
    {
        ...
        I0I3CalculationKernel2 << <blocks * FACTOR5I0I3, THREADS5I0I3 >> > (
            ...
            (float2*)I3d,
            (real2*)ptr.Mlowerl, (real2*)ptr.Mupperl,
            epsastd, meanEpsd, npans, (real*)pans, (real*)visstr, (float3*)i0i3Analyt);
