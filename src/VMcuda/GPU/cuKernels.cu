        ...
        do
        {
            ...
            while (pd < 2)
            {
                ...
                if (isVortex)
                {
                    ...
                    gm = dev_ptr_freeVortexSheet[MmortonCodesIdxd[n]];// vtxd[MmortonCodesIdxd[n]].z;
                ...
                if (isVortex || __all_sync(0xffffffff, ((sumSide2.x + sumSide2.y) * (sumSide2.x + sumSide2.y) + dlen2) * itolsqd < r2))
                {
                    if (isVortex)
                    {
                        ...
                        if (dev_ptr_freeVortexSheetLin != nullptr)                      
                            gmlin = dev_ptr_freeVortexSheetLin[MmortonCodesIdxd[n]];                        
...
/******************************************************************************/
        ...
        real* __restrict vellind)
    {
        ...
        MatrToVecCalculationKernel << <blocks * FACTOR5rhs, THREADS5rhs >> > (
            nnodesd, nbodiesd, itolsqd,
            (int2*)ptr.Mchildl, order, (real2*)ptr.momsl,
            (real*)rpnl,
